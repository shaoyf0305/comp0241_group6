...
def test_on_single_image(image_path, ground_truth_path=None, save_prefix='result', 
                         center_weight=0.3, auto_scale_margin=5, use_connected_components=True):
    ...
    axes[1, 0].imshow(edge_viz)
    axes[1, 0].set_title(f'Method 3: Edges Overlay ', # ({len(edge_points_x)} points)
                         fontsize=12, fontweight='bold')
    axes[1, 0].axis('off')
    
    # Show just the edges in white on black
    axes[1, 1].imshow(edges3, cmap='gray')
    axes[1, 1].set_title('Method 3: Edges', fontsize=12, fontweight='bold')
    axes[1, 1].axis('off')
    

    # METHOD 3
    axes[1, 2].imshow(mask3, cmap='gray')
    axes[1, 2].set_title(f'Method 3: Combined', fontsize=12, fontweight='bold')
    axes[1, 2].axis('off')
    ...
    for idx, (mask, name) in enumerate(method_list):
        ...
        axes[2, idx ].imshow(cv2.cvtColor(blended, cv2.COLOR_BGR2RGB))
        axes[2, idx ].set_title(f'{name} Overlay', fontsize=12, fontweight='bold')
        axes[2, idx ].axis('off')
