...
#circle and ellipse--single test
#designed for task1
...
def segment_by_geometry(img):
    ...
    try:
        ...
        # Calculate ellipse fitting error
        # Create temporary mask and compare with contour points
        temp_mask = np.zeros_like(gray)
        cv2.ellipse(temp_mask, ellipse, 255, 1)  # Draw ellipse boundary
        
        # Distance from contour points to ellipse boundary
        distances_ellipse = []
        for pt in pts:
            # Check distance to nearest ellipse pixel
            y_pt, x_pt = int(pt[1]), int(pt[0])
            # Sample around the point to find minimum distance to ellipse
            min_dist = float('inf')
            for dy in range(-5, 6):
                for dx in range(-5, 6):
                    ny, nx = y_pt + dy, x_pt + dx
                    if 0 <= ny < temp_mask.shape[0] and 0 <= nx < temp_mask.shape[1]:
                        if temp_mask[ny, nx] > 0:
                            dist = np.sqrt(dx**2 + dy**2)
                            min_dist = min(min_dist, dist)
            distances_ellipse.append(min_dist)
...
def color_then_circle(image):
    ...
    try:
        ...
        # Calculate ellipse fitting error
        temp_mask = np.zeros_like(ocean_mask)
...
if __name__ == "__main__":
    ...
    test_image = "final_dataset/Easy/images/000001.png"
    test_mask = "final_dataset/Easy/masks/000001.png"
